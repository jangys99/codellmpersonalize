...
class HiePolicy:
    ...
    def reset(self):
        ...
        # 1. graph extract를 위한 코드
        self.step_count = 0
        self.exploration_end_snapshot = None
    ...
    # 2. 그래프를 즉시 파일로 저장하는 함수
    def _save_graph_immediately(self):
        if self.is_graph_saved:
            return

        save_dir = "/workspace/scene_graph"
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)

        try:
            ep_id = self.envs.current_episodes()[0].episode_id
            scene_id = self.envs.current_episodes()[0].scene_id.split('/')[-1].split('.')[0]
        except:
            ep_id = "unknown_ep"
            scene_id = "unknown_scene"

        filename = f"graph_{scene_id}_{ep_id}.json"
        filepath = os.path.join(save_dir, filename)

        try:
            # 현재 시점의 그래프 데이터 가져오기
            graph_data = self.context_module.house_graph.graph
            
            with open(filepath, 'w') as f:
                json.dump(graph_data, f, indent=4)
            
            print(f"[Graph Saved] Exploration finished (Plan 1 done). Saved to: {filepath}")
            self.is_graph_saved = True
